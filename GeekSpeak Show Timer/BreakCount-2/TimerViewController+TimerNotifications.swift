...
// TODO: Move to new TimerView
  ...
  func displayAllTime() {
    ...
    timerViews?.ring2fg.progress = percentage1
    ...
    timerViews?.ring3fg.progress = percentage2
  ...
  @objc func timerUpdatedTime() {
    if let timer = timer {
      ...
      let labelText = "Total: \(totalTime)".pad(" ", totalLength: 15, side: .left)
      ...
      case .Section1:
        ...
        timerViews?.ring1fg.progress = 0.0
        timerViews?.ring2fg.progress = timer.percentageCompleteUnlimited //timer.percentageComplete(.Section1)
        timerViews?.ring3fg.progress = 0.0
